    ...
    try {
        if (is_full_info) {
            string result_file_fullinfo =
                    "results/CCE_Instant/regrets_" + para_file_name + "/" + "fullinfo_" + to_string(exp_idx) + ".txt";
            ...
            vector<int> timestep;
            vector<double> lower_bound(action_size, 1.0 / action_size);
            ...
            for (int t = 1; t <= T; t++) {
                ...
                for (int player = player_num - 1; player >= 0; player--) {
                    ...
                    auto new_permutation = SamplePermutationSingle(single_utilities[player], sampled_actions[player], gen);
                ...
                for (int player = player_num - 1; player >= 0; player--) {
                    vector<double> estimated_utility = chernoff_estimate(t, players[player].estimation_exp_sum,
                                                                         players[player].m1_cnt);
                    UpdateStrategyPGD(players[player].strategy, estimated_utility, 1.0, gamma_v,
                                      lower_bound);
                }
            ...
            string result_file_bandit =
                    "results/CCE_Instant/regrets_" + para_file_name + "/" + "bandit_" + to_string(exp_idx) + ".txt";
            ...
            for (int t = 1; t <= T; t++) {
                ...
                for (int player = player_num - 1; player >= 0; player--) {
                    vector<double> estimated_utility = chernoff_estimate(t, players[player].estimation_exp_sum, players[player].m1_cnt);
                    players[player].strategy = UpdateStrategyPGD2(players[player].alg_strategy, estimated_utility, 1.0, gamma_v, lower_bound, t);
                }
                if (t % print_per_round == 0) {
                    double avg_fullinfo_regret = 0.0;
                    for (int player = 0; player < player_num; player++) {
